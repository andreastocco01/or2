...
\begin{itemize}
	...
	non-tabu moves
\end{itemize}
% With this approach we waste a lot of time without producing any better 
% solution (Diversification Phase). One possible solution could be restarting from a random point every time we 
% find a local minimum (Multistart) but, in this way, we would spend a lot of time doing the Intensification 
% Phase. Furthermore, when we reach a local minimum it's very likely that the solution found is not 
% completely wrong, but that has to be `fixed` in some sense.
% VNS is a metaheuristic that performs a little permutation of the solution to escape from a local minimum
% instead of restarting from a completely different one.
% With this approach we move from one solution to a better one using 2-opt until we find a local minimum.
% At this point we perform a permutation of three nodes (3-opt) to escape from the minimum.
% In this way we also ensure that we'll never restart from a solution already visited in the last 
% 2-opt application because it cannot be obtained by a single permutation of 3 nodes.
...
a local minimum. Subsequently, we execute a permutation involving three nodes (3-opt) to navigate away 
from the minimum. This approach also guarantees that we never revert to a previously visited solution in 
the last 2-opt application, as it cannot be achieved through a single permutation of three nodes.
