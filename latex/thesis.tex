...
solutions for large instances, and advanced technique such as branch-and-cut and
...
with fixed tenure.
The Tabu Search with sinusoidal tenure emerged as the best heuristic, mantaining superior
performance across nearly all instances, as shown in the performance profiles.
In our comparison of exact method, we observed that provide the solver with heuristic solutions
during the optimization process and using fractional cuts through the Concorde interface
were more effective than using a warm start.
The best results were achieved by combining all these techniques, as evinced by the
performance profiles.
We also observed that matheuristics are more effective than a pure Tabu Search approach,
demonstrating that the combination of heuristic solutions and exact methods can be a winning
