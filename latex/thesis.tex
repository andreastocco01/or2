...
% constraint can be reduced to a maximum flow problem} \cite{}.
We use a routine from the Concorde software to solve the maximum flow problem
corresponding to the LP relaxation of our TSP problem. This involves functions like \verb|CCconnect_components|,
which identifies connected components in the graph, and \verb|CCviolated_cuts|, which generates violated cuts.
By using these functions we can add SECs derived from the fractional solution of the LP relaxation, rather than from the integer solution.
