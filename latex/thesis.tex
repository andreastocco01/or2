...
Algorithm \ref{alg:diving} shows an abstract implementation of the procedure.  

\begin{algorithm}[ht]
\caption{Diving}
\label{alg:diving}
\begin{algorithmic}
\Procedure{diving}{$model$}
        \State $bestsolution \gets \Call{greedy2opt}$
        \While{$!stop$}
                \State $\Call{fixedges}{model}$
                \State $solution \gets \Call{mipsolver+callback}{model}$
                \If{$\Call{cost}{solution} < \Call{cost}{bestsolution}$}
                        \State $bestsolution \gets solution$
                \EndIf
                \State $\Call{unfixedges}{model}$
        \EndWhile
