...
int CCcut_violated_cuts(int ncount,
			int ecount,
			int* elist,
			double* dlen,
			double cutoff,
			...
			void* pass_param)
{
	...
	rval = mincut_work(ncount, ecount, elist, dlen, (double*)NULL, (int**)NULL, (int*)NULL, cutoff, doit_fn,
			   pass_param);
	if (rval) {
	...
	if (x) {
		for (i = 0; i < ecount; i++) {
			if (x[i] > CONNECT_ZERO_EPSILON) {
				...
				nodelist[elist[2 * i + 1]].adj[nodelist[elist[2 * i + 1]].degree++] = elist[2 * i];
			}
		}
	} else {
		for (i = 0; i < ecount; i++) {
...
static int mincut_work(int ncount,
		       int ecount,
		       int* elist,
		       double* dlen,
		       double* cutval,
		       ...
		       void* pass_param)
{
	...
	rval = CCcut_SRK_subtour_shrink(&G, &minval, CC_MINCUT_ONE_EPSILON, cb, cut, cutcount);
	if (rval) {
		fprintf(stderr, "CCcut_SRK_subtour_shrink failed\n");
		goto CLEANUP;
	}

	if (CCcut_SRK_grab_edges(&G, &sncount, &secount, &slist, &slen, (CC_SRKexpinfo*)NULL)) {
	...
	while (sncount > 1) {
		if (G.head->adj == (CC_SRKedge*)NULL || G.head->next->adj == (CC_SRKedge*)NULL) {
			fprintf(stderr, "Disconnected graph\n");
			rval = 1;
			goto CLEANUP;
		}
		rval = CCcut_mincut_st(sncount, secount, slist, slen, 0, 1, &val, mytcut, &tcount);
		if (rval) {
		...
		CCcut_SRK_identify_pr_edges(&G, &minval, &i, squeue, CC_MINCUT_ONE_EPSILON, cb, cut, cutcount);

		/* if (i) { printf ("[%d]", i); fflush (stdout); } */

		CC_IFFREE(slist, int);
		CC_IFFREE(slen, double);
		rval = CCcut_SRK_grab_edges(&G, &sncount, &secount, &slist, &slen, (CC_SRKexpinfo*)NULL);
		if (rval) {
			fprintf(stderr, "grab edges failed in shrink_ones\n");
			goto CLEANUP;
		}
...
int CCcut_SRK_subtour_shrink(CC_SRKgraph* G,
			     double* minval,
			     double epsilon,
			     CC_SRKcallback* cb,
			     int** cut,
			     int* cutcount)
{
	...
	rval = CCcut_SRK_identify_pr_edges(G, minval, &k, (CC_SRKnode*)NULL, epsilon, cb, cut, cutcount);
	if (rval) {
...
int CCcut_SRK_grab_edges(CC_SRKgraph* G, int* oncount, int* oecount, int** olist, double** olen, CC_SRKexpinfo* expand)
{
	int rval = 0;
	int k, num;
...
int CCcut_mincut_st(int ncount,
		    int ecount,
		    int* elist,
		    double* ecap,
		    int s,
...
int CCcut_SRK_expand(CC_SRKexpinfo* expand, int* arr, int size, int** pnewarr, int* pnewsize)
{
	int newsize = 0;
	int* newarr = (int*)NULL;
	int i, j, jend;
	...
	*pnewarr = (int*)NULL;
	...
	newsize = 0;
	for (i = 0; i < size; i++) {
		for (j = expand->memindex[arr[i]], jend = expand->memindex[arr[i] + 1]; j < jend; j++) {
...
int CCcut_SRK_identify_pr_edges(CC_SRKgraph* G,
				double* minval,
				int* count,
				CC_SRKnode* qstart,
				double epsilon,
...
#ifdef PR_USE3
			-Must modify to use node current min cut value.for (e = n->adj; e; e = e->next)
			{
				tol = e->weight;
				for (f = e->end->adj; f; f = f->next) {
...
#ifdef PR_USE4
			-Must modify to use node weights.for (e = n->adj; e; e = e->next)
			{
				tol = onetol - e->weight;
				for (f = e->end->adj; f; f = f->next) {
...
static int buildgraph(graph* G, int ncount, int ecount, int* elist, double* ecap)
{
	int i;
	edge* edgelist;
	node* nodelist;
	...
	for (i = 0; i < ecount; i++) {
		...
		if (tail < 0 || tail >= ncount || head < 0 || head >= ncount) {
