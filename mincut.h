...
#define RELABEL_BODY(n)                                                                                                \
	for (rele = (n)->out; rele; rele = rele->outnext) {                                                            \
		if (rele->cap - rele->flow > PRF_EPSILON && (relt = rele->ends[1]->flowlabel) < relm)                  \
			relm = relt;                                                                                   \
	}                                                                                                              \
	for (rele = (n)->in; rele; rele = rele->innext) {                                                              \
		if (rele->cap + rele->flow > PRF_EPSILON && (relt = rele->ends[0]->flowlabel) < relm)                  \
			relm = relt;                                                                                   \
	}                                                                                                              \
	(n)->flowlabel = ++relm;
#else
#define RELABEL_BODY(n)                                                                                                \
	...
	for (rele = (n)->in; rele; rele = rele->innext) {                                                              \
		if (rele->flow > PRF_EPSILON && (relt = rele->ends[0]->flowlabel) < relm)                              \
			relm = relt;                                                                                   \
	}                                                                                                              \
	(n)->flowlabel = ++relm;
#endif
	...
	{                                                                                                              \
		...
		if (relm < ncount) {                                                                                   \
			...
			if (!level[relold]) {                                                                          \
				...
				while (level[relold]) {                                                                \
					node* relno;                                                                   \
					for (relno = level[relold]; relno; relno = relno->levelnext) {                 \
...
static int buildgraph(graph* G, int ncount, int ecount, int* elist, double* gap),
...
static void free_graph(graph* G), connect_search(graph* G, int n, int marker, int* dstack), init_graph(graph* G),
...
extern "C" {
#endif
void CCcut_SRK_init_graph(CC_SRKgraph* G), CCcut_SRK_free_graph(CC_SRKgraph* G),
    CCcut_SRK_init_expinfo(CC_SRKexpinfo* expand), CCcut_SRK_free_expinfo(CC_SRKexpinfo* expand),
    CCcut_SRK_init_callback(CC_SRKcallback* cb), CCcut_SRK_identify_nodes(CC_SRKgraph* G, CC_SRKnode* n, CC_SRKnode* m);
...
int CCcut_violated_cuts(int ncount,
			...
			int (*doit_fn)(double, int, int*, void*),
			void* pass_param),
    CCcut_connect_components(int ncount, int ecount, int* elist, double* x, int* ncomp, int** compscount, int** comps),
    CCcut_SRK_buildgraph(CC_SRKgraph* G, int ncount, int ecount, int* elist, double* dlen),
    CCcut_SRK_subtour_shrink(CC_SRKgraph* G,
			     double* minval,
			     double epsilon,
			     CC_SRKcallback* cb,
			     int** cut,
			     int* cutcount),
    CCcut_SRK_grab_edges(CC_SRKgraph* G, int* oncount, int* oecount, int** olist, double** olen, CC_SRKexpinfo* expand),
    ...
    CCcut_SRK_identify_pr_edges(CC_SRKgraph* G,
				double* minval,
				int* count,
				CC_SRKnode* qstart,
				double epsilon,
